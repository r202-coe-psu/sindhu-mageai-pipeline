...
async def insert_data(metrics_stations):
    ...
    print("### Start insert data (Strict Songkhla Rain Metrics Only)")
    metrics_to_save = []
    exist_counter = 0
    ...
    for code, metrics_station in metrics_stations.items():
        ...
        # ยังไม่มี station ของ source นี้ใน DB ให้ข้าม (ต้องรัน thaiwater_rain_stations ก่อน)
        if not station:
            continue

        print(f"[>] Processing rain metrics for station: {code} - {metrics_station[0]['name_th']}")

        for data in metrics_station:
            station_code = data.pop("code")
            name_th = data.pop("name_th")
            data_source = data.pop("source")
            rainfall_datetime = data.pop("rainfall_datetime")

            # แปลงค่ากลับเป็น datetime object สำหรับบันทึกพิกัดเวลา (Timestamp)
            timestamp = datetime.datetime.fromisoformat(rainfall_datetime)
            ...
            for parameter, value in data.items():
                ...
                # จัดเตรียม Metadata และแก้ไข source ให้เป็น thaiwater_rain ให้ถูกต้อง
                metadata = dict(
                    source="thaiwater_rain",
